...
| `gcdg`      | A model covering 14 instruments using *direct* measurements                                 | @weber2019              |
| `gsed1912`  | Covers 20 instruments using a mix of *direct* and *caregiver-reported* measurements         | @vanbuuren2025 (Study 1)|
| `gsed2406`  | Covers 23 instruments using a mix of *direct* and *caregiver-reported* measurements         | @vanbuuren2025 (Study 2)|
| `gsed2510`  | Currently covers 4 instruments - In development                                             | In preparation (Study 3)|
...
-   To explicitly set the most recent key use `key = "gsed2510"`.
-   Key `gsed2406` has a wider coverage, and use that if your instruments is not covered by `gsed2510`.
...
| `gs1` | GSED SF (2023) | 139 |  |  |  | 138 | + |
| `gl1` | GSED LF (2023) | 155 |  |  |  | 155 | + |
...
Unfortunately, you cannot compute a D-score if your instrument is not yet supported. If your instrument does measure child development, it may be possible to extend the key to include it. This requires some one-time effort (e.g. mapping your items to the milestone schema, estimating difficulty parameters). Once extended, the key can be reused, and you’ll be able to compute valid D-scores for that instrument. The table shows two extensions: One for Mullen and and one for ECDI.
