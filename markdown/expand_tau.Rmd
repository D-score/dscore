...
need to be anchored by means of the `anchor_dscore()` function.

```{r fit1}
fit1 <- rasch(select(smocc, -(1:5)))
tau1 <- anchor_dscore(get_diff(fit1), anchor_items = c("n12", "n26"))
round(tau1, 1)
```
...
tau2 <- anchor_dscore(get_diff(fit2), anchor_items = c("n12", "n26"))
round(tau2)
```
...
tau3 <- anchor_dscore(get_diff(fit3), anchor_items = c("n12", "n26"))
round(tau3, 1)
```
...
# tau4 <- anchor_dscore(get_diff(fit4), anchor_items = c("n12", "n26"))
```

Because the anchor items were not observed in the `doove` data, `anchor_scale()`
