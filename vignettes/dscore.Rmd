...
```{r setup, include=FALSE}
  knitr::opts_chunk$set(echo = TRUE, fig.retina = 2, eval = FALSE)
  options(width = 80)
```
...
The names of the items in the data need to be matched against one of the lexicons in the item bank. The item bank in the `dscore` package is called `builtin_itembank`. For each item, the item bank contains: 
...
The source data has now names that are recognized in the itembank. To check this, find the difficulties for each item by the `get_tau()` function:

```{r}
get_tau(items = names(popsdemo)[item_locations])
```
...
```{r eval=FALSE}
child1 <- dplyr::filter(data, patid == 11126)
