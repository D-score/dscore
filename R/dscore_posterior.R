#' D-score estimation that returns the posterior estimates
#' 
#' This function is a copy of \code{"dscore"} that estimates the D-score, a
#' numeric score that measures child development, from PASS/FAIL observations on
#' milestones. This function returns the posterior distribution for the D-score
#' estimate.
#' 
...
#' \code{dec = 3}.
#' @return 
#' A list with two elements. Element \code{dscore} contains a
#' \code{data.frame} with \code{nrow(data)} rows and the following 
#' columns:
#' \describe{
...
#' }
#' and element \code{posterior} contains two large lists, one with a list of
#' \code{nrow(data)} elements with starting prior and a list of
#' \code{nrow(data)} elements with posterior.
#' 
#' @details
...
#'items <- names(data)[6:62]
#'ds <- dscore_posterior(data=data, items=items)
#'ds$dscore
#'}
#'
dscore_posterior <- function(data, 
  ...
  data3l <- data2 %>%
    ...
    summarise(
      ...
      x = list(qp),
      s = list(calculate_posterior(scores = .data$score, 
                                            tau = .data$tau, 
                                            qp  = qp,
                                            mu  = (.data$mu)[1],
  ...
  dscore <- data.frame(.rownum = 1L:nrow(data)) %>%
  ...
  # return full posterior and eap as list
    post <- list(start = data3l$s, posterior = data3l$w)

  
   return(list(dscore = dscore, post = post))
