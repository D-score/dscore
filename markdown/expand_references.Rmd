---
title: "D-scores and references, ages 2-4 years"
...
The vignette `dmetric::D-score_expand.html` describes how the Rasch model 
...
Data on 18 common items and 20 new items were collected by Bernice Doove, and
can be found in the `NL2` object of the `ddata` package. The new variables are 

```{r items}
itemnames <- names(NL2)[names(NL2) %in% itemtable$item]
itemnames
```
...
varnames <- c("id", "wave", "age", itemnames)
nl2_dscore <- NL2 %>%
  mutate(age = agedays / 365.25) %>%
  select_(.dots = varnames) %>%
  gather_("item", "score", itemnames, na.rm = TRUE) %>%
  arrange(id, age) %>%
  group_by(id, age) %>%
  summarise(d = dscore(score, item, age, mu = "model", lexicon = "jam")) %>%
  ungroup()
